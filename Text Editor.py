...
root.iconbitmap('icons/mystif.ico')
...
def open_():
    file = filedialog.askopenfile(initialdir="/", title='Select a text file',
                                  filetypes=[("Text file", "*.txt"), ("All files", "*.*")])
    content_text.delete('1.0', END)
    root.title(os.path.basename(file.name) + " - Mystifv1.0 Text Editor")
    if file != None:
        content = file.read()
...
def save_():
    file = filedialog.asksaveasfile(mode="w", defaultextension=".txt",
                                    filetypes=[("HTML file", "*.html"), ("Text file", "*.txt"), ("All files", "*.*")])
    if file != None:
...
def about_():
    lbl = messagebox.showinfo("About", "Text Editor by Prakhar Saxena.")
    lbl.pack()
...
new_file_icon = PhotoImage(file='icons/add.png')
open_file_icon = PhotoImage(file='icons/open.png')
...
editMenu.add_command(label='Cut',  image=cut_icon, underline=0, command=cut,accelerator='Cut')
editMenu.add_command(label='Copy', accelerator='Copy', image=copy_icon, underline=0, command=copy)
editMenu.add_command(label='Paste', accelerator='Paste',  image=paste_icon, underline=0, command=paste)
